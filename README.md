...
- **9 Ready-to-use Stack Options**: React, Express, Expo, Fiber, Go `net/http`, Django, and FastAPI
...
- **Authentication Boilerplate**: JWT patterns in the Express, Fiber, Go `net/http`, and Django templates
...
| **Go net/http + SQLC** | Go standard-library API with generated database queries | net/http, SQLC, pgx, PostgreSQL, JWT |
...
### Backend (Go net/http + SQLC)

- Go standard-library **net/http** server and route patterns
- **SQLC-generated** type-safe PostgreSQL queries
- **pgx** connection pooling
- **bcrypt** password hashing
- **JWT cookie authentication** with register, login, current-user, and logout routes
- Automatic access-token renewal from the refresh-token cookie
- Focused controller, middleware, route, and utility packages
...
Make sure PostgreSQL is running and the `DATABASE_URL` in `.env` points to it.
