...
import { ArrowRight, LayoutTemplate, Terminal, Zap } from "lucide-react"

const Pill = ({ label }: { label: string }) => (
  <span className="rounded-full border border-border bg-card/80 px-3 py-1 text-[11px] font-mono text-muted-foreground shadow-sm shadow-black/5 backdrop-blur">
    {label}
  </span>
)
    ...
    <div className="min-h-screen bg-gradient-to-b from-background via-background/90 to-background/60 text-foreground">
      ...
      <header className="mx-auto flex w-full max-w-5xl items-center justify-between px-6 py-6 md:py-8">
        ...
        <nav className="hidden items-center gap-4 text-xs font-medium text-muted-foreground md:flex">
          ...
          <a
            ...
            href="https://github.com/RajanDhamala/cli"
            rel="noreferrer"
            target="_blank"
          >
            <svg
      ...
      <main className="mx-auto flex w-full max-w-5xl flex-col gap-10 px-6 pb-16 pt-2 md:flex-row md:items-center md:gap-12 md:pb-24">
        <section className="flex-1 space-y-7">
          <div className="inline-flex items-center gap-2 rounded-full border bg-card/80 px-3 py-1 text-[11px] font-medium text-muted-foreground shadow-sm shadow-black/5 backdrop-blur">
          ...
          <div className="space-y-3">
            <h1 className="bg-gradient-to-b from-foreground to-foreground/70 bg-clip-text text-4xl font-semibold leading-tight tracking-tight text-transparent sm:text-5xl md:text-5xl">
            ...
            <p className="max-w-xl text-sm text-muted-foreground sm:text-base">
              ...
              batteries‑included, and styled like a modern React starter —
              without the noise.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <a
              className="group inline-flex items-center justify-center gap-2 rounded-full bg-foreground px-5 py-2.5 text-sm font-semibold text-background shadow-[0_18px_45px_-18px_rgba(0,0,0,0.7)] transition hover:-translate-y-[1px] hover:bg-foreground/95"
              href="https://www.npmjs.com/package/letsinit"
              rel="noreferrer"
              target="_blank"
            >
